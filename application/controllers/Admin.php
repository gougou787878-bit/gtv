...
class AdminController extends \Yaf\Controller_Abstract
{
    ...
    public function upload_bak($img, $position = 'ads', $name = 'img')
    {
        ...
        $return = $this->uploadImg($img_id, $image_file, $type, $position, config('upload.mp4_upload'));
        if ($return['code'] == 1) {
            unlink($image_file);
    ...
    public function upload($img, $position = 'ads', $name = 'img')
    {
        ...
        $return = $this->uploadImg($img_id, $img, $type, $position, \Yaf\Registry::get('config')->upload->img_upload);
