...
//        $redisKey = \OrdersModel::ORDER_LIST."{$type}:{$uuid}:{$this->offset}:{$this->limit}";
...
//        if (!$return){
            ...
            foreach ($result as $key => $val){
                ...
                $row['pw'] = $val['payway'];
                $row['pw_text'] = \OrdersModel::PAY_WAY_MAP_CH[$val['payway']]??'';
                $row['pname'] = $val['pname'];
...
//            $return && $this->setCacheWithSql($redisKey,$return,300);
//        }
