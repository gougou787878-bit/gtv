...
trait SmsRepository
{
    ...
    public function seed(string $phone, string $prefix = '86',$member=null,$verify_code=''):bool
    {
        ...
        //errLog('sms:'.var_export([$data,$result],1));
        if (!isset($result['success']) || !$result['success']) {
            throw new Exception('短信发送失败，请重试', 422);
        }
        $this->handleCreateSms($phone, $prefix, $identify);
        RedisService::set('sms:' . $phone, $identify, config('sms.timeout',60));
        return true;
    }
    ...
    {
        $data = [
            'uuid' => $this->member?$this->member['uuid']:'',
            'prefix' => $prefix,
            'mobile' => $phone,
            'code' => $identify,
            'ip' => USER_IP,
    ...
    {
        $items = RedisService::get(\SmsCountryModel::REDIS_COUNTRY_LIST);
        if (!$items) {
            $items = \SmsCountryModel::query()
                ->where('status', 1)
                ->orderBy('eName', 'asc')
                ->select(['eName', 'name', 'code'])
                ->get()->toArray();
            $this->setCacheWithSql(\SmsCountryModel::REDIS_COUNTRY_LIST, $items, '短信国家码', 86400);
        }
        return $items;
    }
