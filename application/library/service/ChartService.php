...
class ChartService
{
    ...
    public static function chartNewUser() {
        ...
        $rs =  self::getContentData($file, true);
        if($rs['data']){
            collect($rs['data'])->map(function ($_v,$_d){
                \DailyStatModel::initImportHistory(['day'=>$_d,'user'=>$_v]);
            });
        }
        return $rs;
    }
    ...
    public static function chartInviteNewUser() {
        ...
        if($rs['data']){
            collect($rs['data'])->map(function ($_v,$_d){
                \DailyStatModel::initImportHistory(['day'=>$_d,'tui_user'=>$_v]);
            });
        }
        return $rs;
    }
    ...
    public static function chartActiveUser() {
        ...
        if($rs['data']){
            collect($rs['data'])->map(function ($_v,$_d){
                \DailyStatModel::initImportHistory(['day'=>$_d,'activity'=>$_v]);
            });
        }
        return $rs;
    }
    ...
    public static function chartOrderCharge() {
        ...
        if($rs['data']){
            collect($rs['data'])->map(function ($_v,$_d){
                \DailyStatModel::initImportHistory(['day'=>$_d,'charge'=>$_v]);
            });
        }
        return $rs;
    }
     ...
     * 用户金币交易额 充值
     */
    public static function chartUserCoinChargeIncome() {
        $file = __FUNCTION__;
        $name = '冲钻额';
        $dateList = self::getDateList();
        $getData = self::getContentData($file);
        ...
        foreach ($dateList as $day) {
            ...
            $end = strtotime($day . ' 24:00:00');
            if (empty($getData) || !isset($getData['data'][$day]) || self::isToday($day) || (!self::isToday($day) && !self::checkBeforeDay($day, $file))) {//不存在 或在当前没有数据
                $number = \UsersCoinrecordModel::where('type', 'income')->where('action', 'recharge')->whereBetween('addtime', [$start, $end])->sum('totalcoin');
                $data[$day] = number_format($number, 2, '.', '');
                self::checkBeforeDay($day, $file, true);
            }
        }
