...
class PictureService
{
    ...
    public static function getDetailData($id)
    {
        ...
        $detail->has_right =  (($detail->coins && $detail->is_pay) || ($detail->coins<=0 && $member->is_vip && $member->vip_level));
        if($detail->has_right){
            $detail->load('series')->series;
        }else{
            $detail->load(['series'=>function($query){
                return $query->limit(self::FREE_SEE_LIMIT);
            }])->get();
        }
        \PictureModel::addView($id);
        return $detail;
    }
    /**
     * @param $comics_id
