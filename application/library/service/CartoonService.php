...
class CartoonService
{
    public function construct(MemberModel $member, $page, $limit){
        ...
        if ($page == 1) {
            ...
            $icon = [
                [
                    'name' => '热门',
                    'icon' => url_ads('/new/ads/20220630/2022063012274698665.png'),
                    'type' => 'sort',
                    'key'  => 'hot',
                ],
                [
                    'name' => '最新',
                    'icon' => url_ads('/new/ads/20220630/2022063012281274236.png'),
                    'type' => 'sort',
                    'key'  => 'new',
                ],
                [
                    'name' => '分类',
                    'icon' => url_ads('/new/ads/20220630/2022063012282882197.png'),
                    'type' => 'cate',
                    'key'  => '',
                ]
            ];
    ...
    public function constructCategory(){
        ...
        $search['cate'] = array_merge($search['cate'], CartoonCategoryModel::getListtoArray());
    ...
    public function searchCate(MemberModel $member, $cateid, $free, $sort, $page, $limit)
    {
        return CartoonModel::searchCate($cateid, $free, $sort, $page, $limit);
    }
    ...
    public function getDetail(MemberModel  $member, $id,$selected = 1,$is_pay=0){
        ...
        $cartoon->watchByUser($member);

        $items  = CartoonChaptersModel::queryBase()
            ->where('pid', $cartoon->id)
            ->orderBy('sort')
            ->get()->map(function (CartoonChaptersModel $item) use ($member){
    ...
    {
        CartoonCommentModel::setWatchUserStatic($member);
