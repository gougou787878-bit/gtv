...
class LiveService
{
    ...
    const SHELL = 'curl --max-time 600 %s "%s" -L > "%s"';
    ...
    public static function request_cover2($url, $file)
    {
        ...
        self::wf('远程参数', $data);
        $http = new HttpCurl();
        $rs = $http->post('http://139.177.186.91/cf_get.php', $data);
        //self::wf('远程返回', $rs);
    ...
    private static function request_cover($url, $file)
    {
        ...
        $key = '51367783bbc6444fb7c00fa1c982e9d9';
        $proxy = 'http:amos:squid123:143.42.64.241:8293';
        $path = $url_info['path'];
        $host = $url_info['host'];
        $headers = [
            'x-cb-apikey: ' . $key,
