...
class PornGameService extends \AbstractBaseService
{
    ...
    public function construct(MemberModel $member, $page, $limit){
        ...
        //标签
        $flag = PornCategoryModel::getFlagArray();
    ...
    public function detail(MemberModel $member, $id){
        ...
        //用户免费次数
        $free_num = 0;//UsersProductPrivilegeModel::hasPrivilege(USER_PRIVILEGE, PrivilegeModel::RESOURCE_TYPE_PORN_GAME, PrivilegeModel::PRIVILEGE_TYPE_UNLOCK);
        return [
            'ads'    => $ads,
            'detail' => $detail,
        ...
        $flag = transaction(function () use ($member, $record, $game, $porn_id) {
            ...
            if ($record) {
                ...
                bg_run(function () use ($porn_id){
                    PornGameModel::decrByLike($porn_id);
                });
                return 0;
            } else {
                ...
                bg_run(function () use ($porn_id){
                    PornGameModel::incrByLike($porn_id);
                });
                return 1;
            }
        });
    ...
    // 收藏
    public function favorite(MemberModel $member, $porn_id){
        $game = PornGameModel::detail($porn_id);
        test_assert($game, '黄游不存在');
        $record = PornFavoriteModel::where('aff', $member->aff)->where('porn_id', $porn_id)->first();
        $flag = transaction(function () use ($member, $record, $game, $porn_id) {
            $key = sprintf(PornFavoriteModel::PORN_GAME_FAVORITE_SET, $member->aff);
            if ($record) {
                $itOk1 = PornFavoriteModel::where(['aff' => $member->aff, 'porn_id' => $porn_id])->delete();
                test_assert($itOk1, '操作失败');
                //从redis里面移除
                redis()->sRem($key, $porn_id);
                bg_run(function () use ($porn_id){
                ...
                $record = PornFavoriteModel::create([
                    'aff' => $member->aff,
                    'porn_id' => $porn_id,
                    'created_at' => \Carbon\Carbon::now(),
                    'updated_at' => \Carbon\Carbon::now(),
    ...
    public function listFavorite(MemberModel $member, $page, $limit){
        $list = PornFavoriteModel::list($member->aff, $page, $limit);
        if ($list) {
            $list = $this->v2format($list);
        }
        return $list;
    ...
    public function buy(MemberModel $member, $porn_id, $type){
        return transaction(function () use ($member, $porn_id, $type) {
            ...
            test_assert($game, '黄游不存在');
            $porn_type = $game->type;
            test_assert($porn_type == PornGameModel::TYPE_COINS, '金币类型才能解锁');
            $is_pay = PornPayModel::hasBuy($member->aff, $porn_id);
            test_assert(!$is_pay, '请勿重复购买');
            ...
            $total = $game_coins;
            $tips = "[购买黄游]{$game->name}";
            test_assert($member->coins > $game_coins, '金币余额不足', 1008);
