...
class CommunityService
{
    ...
    function listFollowPosts(MemberModel $member, $page, $limit)
    {
        $banner = AdService::getADsByPosition(AdsModel::POS_COMMUNITY_BANNER);
        $uidAttention = UserAttentionModel::getList($member);
        $ids = $uidAttention ? collect($uidAttention)->pluck('touid')->toArray():[1];
        if (!$ids) {
            return [];
        }
        $list = PostModel::queryBase()->with('topic:id,name')
            ...
            ->forPage($page, $limit)
            ->get();
        $list = $this->formatPost($list, $member);
        return [
            'banner' => $banner,
            'list'   => $list
        ];
    ...
    function listFollowTopics(MemberModel $member, $page, $limit)
    {
        $banner = AdService::getADsByPosition(AdsModel::POS_COMMUNITY_BANNER);
        $list = PostTopicUserLikeModel::getFollows($member->aff, $page, $limit);
        return [
            'banner' => $banner,
            'list'   => $list
        ];
...
//        }
    ...
    {
        $list = cached('post_topic_cat')
            ->group('post_topic_cat')
            ->chinese('分类列表')
            ->fetchPhp(function () {
                $collect = PostTopicCategoryModel::query()
                    ->where('status', PostTopicCategoryModel::STATUS_NORMAL)
                    ->orderByDesc('sort')
                    ->get();
                return collect2raw($collect);
            });
        return PostTopicModel::makeCollect($list);
    }

    // 结构详情
    public function list_construct(MemberModel $member, $category_id, $sort, $page, $limit)
    {
        $banner = AdService::getADsByPosition(AdsModel::POS_COMMUNITY_BANNER);
        $list = [];
