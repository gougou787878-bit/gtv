    ...
    public function createFind(\MemberModel $member, $title, $images, $coins = 0,$vid = 0)
    {
        transaction(function () use ($member, $title, $images, $coins,$vid){
            if ($coins > 0){
                ...
                \UsersCoinrecordModel::createForExpend('findMv', $member->uid, 0,
                    $coins,
                    0,
                    0,
                    0,
...
//        }
    ...
    public function getReplyByFindByUuid(\MemberModel $getMember, string $uuid, $page, $limit)
    {
        $mv_list = \FindReplyMvModel::getMvListByUuid($uuid, $page, $limit);
        return (new MvService())->formatList($mv_list, $getMember);
    }
    ...
    public function getReplyRank($day,$top = 10){
        ...
        $data = cached($key)
            ...
            //return \MemberRankModel::getRankByDay($day,$top);
        },1000);
        if($data){
            return $data;
        }
        ...
        return cached('find:reply:rank:all')
            ...
            //return \MemberRankModel::getAllReplyRank($top);
        });
    }
    ...
    public function doAccept($findObj, FindReplyModel $replyObj){
        ...
        transaction(function () use ($findObj,$replyObj,$row,$coins){
            ...
            if($coins){
                // add 0627
                //\MemberRankModel::addMemberRank($replyObj->uuid,\MemberRankModel::FIELD_COINS, $coins);
                //\MemberRankModel::addMemberRank($replyObj->uuid,\MemberRankModel::FIELD_RECEIVE);//每日接单排行榜统计
    ...
    public function reward(\MemberModel $member, $reply_id, $coins)
    {
        ...
        transaction(function () use ($member, $toMember,$need_coin,$reach_coin,$reply){
            ...
            \UsersCoinrecordModel::createForExpend('buymv', $member->uid, $toMember->uid,
                $total,
                $reply->id,
                0,
                0,
