...
function getPlayUrlPwa($m3u8, $short = false)
{
    ...
    $long = TB_VIDEO_PWA_CN;//国内加密 加速 完整  pwa 不分国内外
    $m10 = TB_SHORT_VIDEO_PWA_CN;//国内加密 加速 预览host
    $url = ($short ? $m10 : $long) . $m3u8;
...
function url_video_short($path): string
{
    if ('CN' == USER_COUNTRY) {
        if (APP_TYPE_FLAG == 0) {
            $base = TB_SHORT_VIDEO_PWA_CN;//pwa不区分国内外
        } else {
            $base = TB_SHORT_VIDEO_APP_CN;//国内加密 加速 默认
        }
    } else {
        if (APP_TYPE_FLAG == 0) {
            $base = TB_SHORT_VIDEO_PWA_US;//pwa不区分国内外
        } else {
            $base = TB_SHORT_VIDEO_APP_US;//国内加密 加速 默认
        }
    }
...
function getPlayUrl($m3u8, $short = false)
{
    ...
    $long = TB_VIDEO_APP_CN;//国内加密 加速 完整host
    $m10 = TB_SHORT_VIDEO_APP_CN;//国内加密 加速 预览host
...
{
    $shareUrl = setting('global.share', 'https://app.gtav.info,https://club.gtav.info');
...
function getShareDataByAff($aff_code, $channel = '', $flag = false)
{
    ...
    $data['share_url'] = sprintf(SHARE_TMP,$data['share_link']);
    return $data;
}
...
function checkWhiteList()
{
    $whiteList = cached('bkwhitelist')->serializerJSON()->expired(600)->fetch(function () {
        $whiteList = explode(',', (new \tools\HttpCurl)->get('https://white.yesebo.net/ip.txt'));
        return $whiteList;
    });
