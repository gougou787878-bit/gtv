...
{
    $key = SettingModel::getSettingCacheKey();
    static $setting = null;
    if ($setting === null) {
        try{
            $setting = redis()->hGetAll($key);
            CacheKeysModel::createOrEdit($key,'配置');
        }catch (\Throwable $exception){
            errLog(sprintf("%s(%d):%s",$exception->getFile() , $exception->getCode() , $exception->getMessage()));
            return null;
        }
    }
...
function url_svga($url)
{
    $base = config('img.img_svga_url', 'http://web.dzxxyjx.com/svga/');
    return url_resource($url, $base);
...
function url_video($url, $isLocal = false)
{
    ...
    if ($isLocal) {
        // 返回的是 http的   限内部使用
        return url_resource($url, TB_VIDEO_ADM_US);
    }
    ...
    //new
    $com_url = url_resource($url, TB_VIDEO_PWA_CN);
...
function url_video_creator($url)
{
    ...
    $hashkey = baiduHash($com_url,parse_url($url,PHP_URL_PATH));
    $return_url = $com_url . '?' . $hashkey;
    return $return_url;
...
function url_videoMP4($url)
{
    ...
    //errLog("mp4".request()->getModuleName());
    //if (request()->getModuleName() == 'Admin') {
        return url_resource($url, TB_CHECK_VIDEO);
    //}
    return $url;
}
...
//    $url = trim($url, '/');
//    $domain = cfg_site('book.cn');
//    return $domain . '/' . $url . '?v=1';
    return config('app_store_url').'/'.ltrim($url,'/');
}