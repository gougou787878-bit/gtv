...
class CacheDb
{
    ...
    /**
     * 默认 不使用缓存
     * @param bool $isUse  默认true  强制读取db数据 即（不使用缓存）
     * @return CacheDb
     */
    public function usingFuck($isUse = true)
    {
        $this->_usingFuck = $isUse;
        return $this;
    }
    ...
    {
        if ($this->_usingFuck) {
            return call_user_func_array($fetchFn, $args);
        }
