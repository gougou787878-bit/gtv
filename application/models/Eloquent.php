...
class EloquentModel extends Model
{
     ...
     * @return bool
     */
    public function toggleColumn( string $column, array $values): bool
    {
        $values = array_merge($values, $values);
        $key = array_search($this->{$column}, $values);
        reset($values);
        $this->{$column} = $key === false ? current($values) : $values[$key + 1];
        return $this->save();
    }
    ...
    /**
     * @var array|MemberModel 观看的用户
     */
    protected $watchUser = null;
     ...
     */
    protected static $watchUserStatic = null;

    public static function setWatchUserStatic(?MemberModel $watchUser)
    {
        self::$watchUserStatic = $watchUser;
    }
