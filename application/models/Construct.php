...
class ConstructModel extends EloquentModel
{
    ...
    const NSG_TAB_LIST = "nag:tab:list:%s:%s:%s";
    ...
    public static function getMvByTag(MemberModel $member, \NavigationModel $nag, $page){
        $limit = 5;
        $nag_id = $nag->id;
        $cacheKey = sprintf(self::NSG_TAB_LIST,$nag_id, $page, $limit);
        $data = cached($cacheKey)
            ->group(self::NSG_TAB_LIST_GROUP)
            ->chinese('导航-视频分类')
            ->fetchPhp(function () use($nag_id, $page, $limit){
                return self::queryBase()
                    ...
                    ->map(function (ConstructModel $item) {
                        ...
                        switch ($item->type) {
                            case self::TYPE_COMMON:
                                //普通
                                $mvData = MvModel::getHomeMvDataByTag($item->id, 'like', 1 , 30);
                                break;
                            case self::TYPE_LIKE:
                                //大家都喜欢
                                $mvData = MvModel::getHomeMvDataByTag(0, 'like',1,30);
                                break;
                            case self::TYPE_HOT:
                                //最热
                                $mvData = MvModel::getHomeMvDataByTag(0, 'hot',1,30);
                                break;
                            case self::TYPE_SALE:
                                //畅销榜
                                $mvData = MvModel::getHomeMvDataByTag(0, 'sale',1,30);
                                break;
                            case self::TYPE_NEW:
                                //最新
                                $mvData = MvModel::getHomeMvDataByTag(0, 'new',1,30);
                                break;
                            default:
                                break;
                        }
