 ...
 * @property string keep_1day_rate 1日留存率
 * @property int keep_3day 3日留存
 * @property string keep_3day_rate 3日留存率
 * @property int keep_7day 7日留存
 * @property string keep_7day_rate 7日留存率
 *
 * @author xiongba
...
class DailyStatModel extends Model
{
     ...
     */
    static function initImportHistory($data = [])
    {
        $initData = [
            'day' => date('Ymd'),
            'ip' => 1,
        ...
        $day = $initData['day'];
        if ($has = self::useWritePdo()->where(['day' => $day])->first()) {
            unset($data['day']);
    ...
    public static function countSystemNew($day = null)
    {
        date_default_timezone_set('Asia/Shanghai');
        $date = date('Y-m-d', strtotime('-1 day'));//前一天
        $day && $date = $day;
        ...
        //渠道访问
        $tui_user = SysTotalModel::getValueBy('channel:welcome', $date);
        //推广独立ip 来源
