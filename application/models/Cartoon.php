...
class CartoonModel extends EloquentModel
{
    ...
    const SHOW_COLUMS = ['id','title','play_count','created_at','cover','is_series','tags','is_free'];
    ...
    public static function searchCate($cate_id, $free, $sort, $page, $limit){
        return cached(sprintf('cartoon:searchcate:list:%s:%s:%s:%d:%d',$cate_id, $free, $sort, $page, $limit))
            ->fetchPhp(function () use ($cate_id, $free, $sort, $page, $limit) {
                return self::queryBase()
                    ->select(self::SHOW_COLUMS)
                    ->when(!empty($cate_id),function ($query)use($cate_id){
                        return $query->where('category_id', $cate_id);
                    })
                    ->when(!empty($free),function ($query)use($free){
                        //return $query->where('is_free', $free);
                    })
                    ->when($sort=='hot',function ($query){
                        return $query->orderByDesc('like_count');
                    })
