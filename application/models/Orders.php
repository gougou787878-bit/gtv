...
class OrdersModel extends EloquentModel
{
    ...
    /**
     * 判断 直播准入用户 严格 ，非充值会员 不能
     * @param $member
     * @return array
     */
    public static function checkEnterLivePermission($member)
    {
        return [true, 1, 1];//04-29 全部放开
        $uuid = $member['uuid'];
        $hasVipCharge = self::hasChargeVip($uuid);
        if (!$hasVipCharge) {
            $device_uuid = MemberModel::hashByAry($member);
            if ($uuid != $device_uuid) {
                $hasVipCharge = self::hasChargeVip($device_uuid);
            }

        }
        $isVip = $member['expired_at'] > TIMESTAMP;
        $isRealyUser = $hasVipCharge && $isVip;
        /**
         * @param int $tokenType token类型  1：普通注册用户，2：游客
         * @param int $enterLivePermission 进入直播间权限  1：有进入直播间权限，0：没有进入直播间权限
         */
        $tokenType = $isRealyUser ? 1 : 2;
        $enterLivePermission = $isRealyUser ? 1 : 0;
        return [$isRealyUser, $tokenType, $enterLivePermission];
    }
     ...
     */
    static function verifyFrequency(array $where = [], int $limit = 10)
    {
        /* $where = [
             ['uuid', '=', $uuid],
             ['order_type', '=', self::TYPE_GAME],
