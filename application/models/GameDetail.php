...
class GameDetailModel extends Model
{
    ...
    static function getHasGameOrder($member, $limit = 2000)
    {
        ...
        $gameCharge = cached('u:order:g:' . $uuid)->expired(300)
            ->serializerJSON()
    ...
    static function sendOver24HourActive($member, $value = 3)
    {
        ...
        if (!$hasSendGift) {
            (new GameService())->transfer($uid, $value, 'add', "超24小时活动送{$value} ~", GameDetailModel::TYPE_GIFT);
        }
        return true;
    }
    static function sendActive($member, $value = 3)
    {
        ...
        if (!$hasSendGift) {
            (new GameService())->transfer($uid, $value, 'add', "{$value} 活动送~ phone:{$hasPhone}", GameDetailModel::TYPE_GAVE);
        }
        return true;
    }
