...
class PornCategoryModel extends EloquentModel
{
    ...
    const CATE_FLAG_PLAY = 'play';
    const CATE_FLAG_NEW = 'new';
    const CATE_FLAG_HOT = 'like';
    const CATE_FLAG = [
        self::CATE_FLAG_PLAY => "正在玩",
        self::CATE_FLAG_NEW => "最新",
        self::CATE_FLAG_HOT => "最热"
    ];
            ...
            ->fetchPhp(function () use ($page, $limit){
                return self::queryBase()
                    ...
                    ->map(function (PornCategoryModel $item) {
                        $porn_data = [];
                        $more_api_params = ['sort' => '', 'id' => $item->id];
//                        switch ($item->type) {
//                            case self::TYPE_COM:
...
//                        }
     ...
     * @return array|array[]
     */
    public static function getFlagArray()
    {
        return array_map(function($id, $val){
            return [
                'key'=>$id,
                'name' => $val
            ];
        }, array_keys(self::CATE_FLAG),self::CATE_FLAG);
    }
}
