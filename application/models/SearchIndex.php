...
class SearchIndexModel extends EloquentModel
{
    ...
    public static function createWord($word, array $keywords, $total, $results)
    {
        if (is_array($results)) {
        ...
        $data = [
            'word'      => $word,
            'word_hash' => self::generateWordHash($word),
            'num'       => 1,
            'total'     => $total,
            'keyword'   => $keywords,
            'results'   => $results
    ...
    public function updateResult($keywords, $vidArray)
    {
        $this->num++;
        if ($this->results != join(',', $vidArray)) {
            $this->results = join(',', $vidArray);
        }
        if (!empty($keywords) && $this->keyword != join(',', $keywords)) {
            $this->keyword = join(',', $keywords);
        }
        $this->save();
    }
...
values (?,?,1,?,?,?,'{$datetime}','{$datetime}') on duplicate key update  num=num+1,keyword=?,results=?,updated_at='{$datetime}'",
    ...
    {
        return self::where('total', '>', 0)->orderBy('num', 'desc')->limit($limit)->get(['word']);
    }
