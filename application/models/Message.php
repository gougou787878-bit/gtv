...
class MessageModel extends Model
{
    ...
    const SYSTEM_MSG_TPL_TOPIC_REFUSE = 'create_top_fail';
    const SYSTEM_MSG_TPL_CREATOR_CHECKING = 'create_apply_ing';
    const SYSTEM_MSG_TPL_SEND_LCG = 'send_vip_lcg';
    //系统消息模板话术
    const SYSTEM_MSG_TPL = [
        self::SYSTEM_MSG_TPL_TOPIC_REFUSE         => '您的剧集：%s 未通过审核,原因：%s',
        ...
        self::SYSTEM_MSG_TPL_CREATOR_CHECKING     => 'UP主(%s)您好，您的申请平台已收到，请尽快加入官方群联系管理员，完善后续视频收益分层保障体系，GTV团队感谢您的支持！',
    ...
    public static function createSystemMessage($uuid, $sys_msg_tpl, $params = [])
    {
        $description = isset(self::SYSTEM_MSG_TPL[$sys_msg_tpl]) ? vsprintf(self::SYSTEM_MSG_TPL[$sys_msg_tpl],
            $params) : '系统通知';
        return self::createMessage(self::SYSTEM_MSG_UUID, $uuid, '系统通知', $description, 0, self::TYPE_SYSTEM,
            self::STAT_ENABLE);
    }
    ...
    static function getMessageList($uuid)
    {
        ...
        $data = cached($key)->serializerJSON()->expired(600)->fetch(function () use ($uuid, $limit, $offset) {
            $data = self::select([
                'id',
                'title',
                'description',
    ...
    static function createSendAppVIPMessage($uuid,$app='xblue')
    {

        if ($app == 'xblue') {
            $url = 'https://aff.gvlan.club/index.php?m=home&a=create_code';
            $timestamp = time();
