...
class UserWithdrawModel extends Model
{
    ...
    protected $fillable = ['uuid', 'cash_id', 'type', 'account', 'name', 'trueto_amount', 'amount', 'status', 'descp', 'created_at', 'payed_at', 'updated_at', 'channel', 'third_id', 'order_desc', 'coins', 'withdraw_type', 'withdraw_from','ip','address'];
    ...
    {
        $isGame = self::DRAW_TYPE_GAME == $withDrawModel->withdraw_from;
        if (!$isGame) {
            return;
        }
        ...
        //errLog($msg,3,APP_PATH . '/storage/logs/draw_'.$dateDay.'.log');
