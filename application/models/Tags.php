...
class TagsModel extends EloquentModel
{
    ...
    static function listData($limit=50)
    {
        return self::where('status','=',self::HOME_YES)
            ->select(['id','name'])
    ...
    {
        redis()->del('pre:upload:tag');//上传
        redis()->del('filter:tags');//筛选 分类 展示
    }

    public static function updateWorksNum(){
        TagsModel::query()
            ->where('status', self::YES)
            ->chunkById(20,function (\Illuminate\Support\Collection $items){
    ...
    public static function getUserUpList(){
        return cached('pre:upload:tag')
            ->fetchPhp(function () {
                return TagsModel::where('user_up', TagsModel::YES)
                    ->orderBy('sort_num')
                    ->pluck('name')
                    ->toArray();
            });
