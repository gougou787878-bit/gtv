...
class PostModel extends EloquentModel
{
    ...
    public function getCreatedAtAttribute()
    {
        ...
        if (MODULE_NAME != 'admin'){
            return $this->attributes['refresh_at'] ?? $created_at;
        }
        return $created_at;
    }
    ...
    public function getIsPayAttribute()
    {
        ...
        if ($this->type == self::TYPE_PAY_FREE){
            return 1;
        }
        ...
        return cached($cacheKey)
            ...
            ->fetchPhp(function () use ($sort, $topic_ids, $page, $limit) {
                return PostModel::queryBase()
                    ->with('topic:id,name')
                    ->with('medias:pid,media_url,cover,thumb_width,thumb_height,type,status')
                    ->when($topic_ids, function ($q) use ($topic_ids){
                        return $q->whereIn("topic_id", $topic_ids);
                    })
                    ->when($sort == 'best', function ($q) {
                        return $q->where('is_best',self::BEST_OK);
                    })
                    ->when($sort == 'hot', function ($q) {
                        return $q->orderByDesc('view_num');
                    })
