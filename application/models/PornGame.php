...
class PornGameModel extends EloquentModel
{
    ...
    public function getIsPayAttribute()
    {
        ...
        if ($type == self::TYPE_COINS){
            //是否是通卡
            $isFreeMember = FreeMemberModel::isFreeMember($this->watchUser->uid);
            if ($isFreeMember) {
                return 1;
            }
            return PornPayModel::hasBuy($this->watchUser->getAttributeValue('aff'), $this->getAttributeValue('id')) ? 1 : 0;
        }
        return 0;
    }
    ...
    public function getIsFavoriteAttribute()
    {
        ...
        $key = sprintf(PornFavoriteModel::PORN_GAME_FAVORITE_SET, $watchUser->aff);
        return redis()->sIsMember($key, $this->attributes['id']) ? 1 : 0;
    }
    ...
    public static function incrByFavorite($id){
        self::where('id', $id)->increment('real_favorite', 1, ['favorite_ct' => DB::raw('favorite_ct + 3')]);
    }
        ...
        return cached($key)
            ...
            ->fetchPhp(function () use($category_id, $sort, $page, $limit){
                return self::queryBase()
                    ...
                    //正在玩
                    ->when('play' == $sort, function ($q){
                        return $q->orderByDesc('buy_fake');
                    })
                    //畅销
                    ->when('sale' == $sort, function ($q){
