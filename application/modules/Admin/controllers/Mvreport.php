...
class MvreportController extends BackendBaseController
{
    ...
    protected function listAjaxIteration()
    {
        return function ($item) {
            ...
            $item->mv_pre_href = url_video($item->mv->m3u8, true);
    ...
    public function delMvAction()
    {
        ...
        if ($flag) {
            ...
            MessageModel::createMessage(MessageModel::SYSTEM_MSG_UUID,$mv->user->uuid , '系统通知' , $desc);
            //举报者
            $desc_reporter =vsprintf('您举报的视频 %s %s,经核实现已被删除', ['title' => $mv->title,'content'=>$model->content]);
            MessageModel::createMessage(MessageModel::SYSTEM_MSG_UUID,$model->uuid, '系统通知',  $desc_reporter);
