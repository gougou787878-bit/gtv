...
class ProductController extends BackendBaseController
{
    ...
    {
        $payway = $this->payWay;
        return function ($item) use ($payway) {
            $pay_name = [];
            if ($item->payway_wechat == 1) {
                array_push($pay_name, $payway['payway_wechat']);
            }
            if ($item->payway_bank == 1) {
                array_push($pay_name, $payway['payway_bank']);
            }
            if ($item->payway_alipay == 1) {
                array_push($pay_name, $payway['payway_alipay']);
            }
            if ($item->payway_visa == 1) {
                array_push($pay_name, $payway['payway_visa']);
            }
            if ($item->payway_huabei == 1) {
                array_push($pay_name, $payway['payway_huabei']);
            }
            if ($item->payway_agent == 1) {
                array_push($pay_name, $payway['payway_agent']);
            }
            if ($item->payway_ecny == 1){
                array_push($pay_name, $payway['payway_ecny']);
            }
