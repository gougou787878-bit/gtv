...
class PostController extends BackendBaseController
{
    ...
    const NOTIFY_PROJECT = [
        ...
        'stweb' => ['name' => '搜同web', 'url' => 'https://stgay.com/sync/melon'],
        'stapp' => ['name' => '搜同APP', 'url' => 'http://soutong-app-index.we-cname2.com/index.php?m=mv&a=syncXlpPostData'],
    ];
    ...
    {
        $pk = $_POST['_pk'] ?? 0;
        $memo = $this->post['refused'] ?? '';
        $custom_reason = $this->post['custom_reason'] ?? '帖子内容描述与小蓝看片平台不符';
        if (empty($memo)){
            $memo = $custom_reason;
        }
        $model = PostModel::find($pk);
    ...
    public function sync_selectAction()
    {
        try {
            ...
            PostModel::with('medias')
                ...
                ->get()
                ->map(function ($item) use ($url, $topicId, $aff, $http, $flag) {
                    ...
                    if ($flag == 'stapp'){
                        $sync_data['time'] = TIMESTAMP;
                        $sync_data['pwd'] = md5('c1999b118f786d90' . $sync_data['time']);
                    }
