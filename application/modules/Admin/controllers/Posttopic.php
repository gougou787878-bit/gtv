...
class PosttopicController extends BackendBaseController
{
    ...
    protected function listAjaxIteration()
    {
        return function (PostTopicModel $item) {
            $item->load('category');
            $item->category_name = isset($item->category) ? $item->category->name : '未设置';
            return $item;
        };
    }
    ...
    {
        $cat_arr = [];
        PostTopicCategoryModel::where('status',PostTopicCategoryModel::STATUS_NORMAL)
            ->get()
            ->map(function (PostTopicCategoryModel $item) use (&$cat_arr){
                $cat_arr[$item->id] = $item->name;
            });
