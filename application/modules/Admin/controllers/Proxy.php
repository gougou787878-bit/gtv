  ...
  /*app_name:nineone
    ...
    public function verifyAction(){
        ...
        if (empty($withraworder)) {
            ...
            $oauth_type = trim($memberinfo['oauth_type']);
            //发起请求
            $data = array(
                "app_id"      => $withraworder['id'],
                "app_name"    => SYSTEM_ID,
                "app_type" => $oauth_type != 'pwa' ? $oauth_type : 'pc',
    ...
    public function forbidAction(){
        $this->showJson("不支持的操作");
        $id = $this->get['id'] ?? 0;
        $withdraw = \UserWithdrawModel::query()->where('id',$id)->first();
        if ($withdraw->status != 5){
            $this->showJson("操作成功",0);
        }else{
            \UserWithdrawModel::query()->where('id',$id)->update(['status'=>0]);
        }
        $this->showJson("操作成功");
    }
...
//            ])
        ...
        $total = $totalproxy = $totalgold = $totalMv = $totalGame = 0;
        ...
        $this->getView()
            ...
            ->assign('withdraw_from', $this->withdraw_from)
