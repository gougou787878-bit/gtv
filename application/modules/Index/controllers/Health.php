...
class HealthController extends Controller_Abstract
{
    ...
    public function init()
    {
       ...
       $database = $object->database->toArray();
       $this->mysql_ip = $database['write']['host'] ?? '';
       $this->mysql_port = $database['port'] ?? '';
       $redis_host = config('redis.host')[0];
       $redis_info = explode(':', $redis_host);
       $this->redis_ip = $redis_info[0];
       $this->redis_port = $redis_info[1];
