...
class ProxyController extends BaseController
{
    ...
    public function withdrawAction()
    {
        if ($this->member['role_id'] == MemberModel::USER_ROLE_LEVEL_BANED) {
            throw new \Yaf\Exception('您已被禁言', 422);
        }
        $member = $this->member;
        ...
        $withdraw_from = (int)$this->post['withdraw_from'] ?? 0;  //1货币2代理 3 视频收益4社区收益
        ...
        if (UserWithdrawModel::DRAW_TYPE_PROXY == $withdraw_from) {
        ...
        if (!is_null($hasCardInfo) && $hasCardInfo->name != $withdraw_name) {
            ...
            throw new \Yaf\Exception("新卡姓名必须与您以前的提现卡姓名({$hasCardInfo->name})⼀致,请填写同名银⾏卡后再提现。如需更改姓名请联系客服。");
        }
        ...
        if (UserWithdrawModel::DRAW_TYPE_MV == $withdraw_from) {//视频收益提现
            ...
            $withdraw_amount_money = $withdraw_amount * UserWithdrawModel::USER_WITHDRAW_MONEY_RATE_MV;
            if ($member->auth_status) {
                $withdraw_amount_money = $withdraw_amount * MemberMakerModel::getMakerRate($member->uuid);
            }
            if($withdraw_amount_money<500){
                \tools\RedisService::del($redisKey);
                throw new \Yaf\Exception('因收益提现通道额度最低限制,500起提谢谢~', 422);
            }
            $trueto_amount = 0;
            try {
                \DB::beginTransaction();
                ...
                // 冻结金额
                $member->score = $member->score - $withdraw_amount;
                $member->save();
                \DB::commit();
            ...
            if($withdraw_amount<200){
                \tools\RedisService::del($redisKey);
                throw new \Yaf\Exception('因代理推广提现通道额度最低限制,200起提谢谢~', 422);
            }
            //总收益
            ...
            try {
                ...
                \UserWithdrawModel::query()->insert($insert_data);
                // 冻结金额
                $member->tui_coins = $member->tui_coins - $withdraw_amount;
                $member->save();
                \DB::commit();
            } catch (Exception $exception) {
                \DB::rollBack();
            ...
            $withdraw_amount_money = $withdraw_amount;
            $trueto_amount = 0;
            try {
                \DB::beginTransaction();
                // 提现记录
