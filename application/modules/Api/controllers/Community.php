...
//        ];
        ...
        return $this->showJson(['ads' => $ads, 'topic' => $hotTopic, 'tab' => $tab,'rank_list' => []]);
    }
...
DOC
    ...
    public function homeAction()
    {
        ...
        return $this->listJson(['ads' => $ads,'tab'=> $tab]);
    }
    ...
    public function list_constructAction()
    {
        try {
            $validator = Validator::make($this->post, [
                'sort' => 'required|enum:recommend,hot,new,video,best', //类型 recommend推荐 new最新 hot最热 best精选 video 视频
                'category_id' => 'required', //分类ID
            ]);
            if ($validator->fail($msg)) {
                throw new Exception($msg);
            }
            $sort = $this->post['sort'] ?? 'recommend';
            $category_id = $this->post['category_id'] ;
            list($page, $limit) = QueryHelper::pageLimit();
            $member = request()->getMember();
            $service = new CommunityService();
            $result = $service->list_construct($member, $category_id, $sort, $page, $limit);
    ...
    public function follow_user_postAction()
    {
        try {
            $member = request()->getMember();
            list($page, $limit) = QueryHelper::pageLimit();
            $result = (new CommunityService())->listFollowPosts($member, $page, $limit);
            return $this->showJson($result);
    ...
    public function follow_topicsAction(){
        try {
            $member = request()->getMember();
            list($page, $limit) = QueryHelper::pageLimit();
            $result = (new CommunityService())->listFollowTopics($member, $page, $limit);
            return $this->showJson($result);
        }catch (Throwable $exception){
            return $this->errorJson($exception->getMessage());
        }
    ...
    public function follow_topic_postAction()
    {
        ...
        $member = request()->getMember();
        $postData = (new CommunityService())->listFollowPosts($member->aff, PostModel::TYPE_FOLLOW_TOPIC,$member);
        return $this->showJson(['post' => $postData]);
    }
}