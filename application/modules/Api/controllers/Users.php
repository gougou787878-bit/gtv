...
class UsersController extends BaseController
{
     ...
     * @throws \Yaf\Exception
     */
    public function register_newAction()
    {
        try {
            ...
            $token = $this->handleRegister($username, $password);
    ...
    public function loginAction()
    {
        ...
        if (strlen($password) < 6) {
            throw new \Yaf\Exception('密码长度不够', 422);
        }

        $user = $this->getUserByPhone($phone);
        if (is_null($user)) {
            throw new \Yaf\Exception('找不到用户', 422);
        }

        if ($user->password != md5($password)) {
            throw new \Yaf\Exception('密码不正确', 422);
        }
    ...
    {
        $aff = $this->post['aff'] ?? '';
        wf('邀请码0', request()->getMember()->uid, false, '/storage/logs/invitation.log');
        wf('邀请码1', $aff, false, '/storage/logs/invitation.log');
        $aff = trim($aff);
        if ($aff == '') {
            $aff = $this->post['aff_code'] ?? '';
            $aff = trim($aff);
            wf('邀请码2', $aff, false, '/storage/logs/invitation.log');
            if (!$aff){
    ...
    public function changePasswordAction()
    {
        ...
        if (strlen($password) < 5) {
            ...
            return $this->errorJson('密码超过20字符,太长了');
        }
    ...
    public function thumbAction()
    {
        ...
        if (!$this->member['vip_level']) {
            return $this->errorJson('仅允许付费会员用户设置头像');
        }
        if ($thumb == '') {
    ...
    public function followingAction()
    {
        ...
        if (!frequencyLimit(20, 4, $member)) {
            return $this->errorJson('短时间内赞操作太頻繁了,稍后再试试');
        }
        if($member->isBan()){
            return $this->errorJson('您已被禁言,联系客服~');
        }
        try {
    ...
    public function fansAction()
    {
        ...
        $items = (new \service\FollowedService())->getUserFansList($uid ,request()->getMember());
        $this->showJson($items);
    }
    ...
    public function videosAction()
    {
        ...
        $items = (new UserVideoService())->getVideosByUid($uid ,$kwy, request()->getMember());
        $this->showJson($items);
    }
    ...
    public function likesAction()
    {
        ...
        $items = (new UserVideoService())->getUserLikesVideoList((int)$uid, $kwy, request()->getMember());
        $this->showJson($items);
    }
    ...
    {
        $currentVersion = $_POST['version'] ?? '1.0.0';
        list($isRealUser, $tokenType, $enterPermission) = OrdersModel::checkEnterLivePermission($this->member);
        $member = request()->getMember();
        $member->append('level', 'level_anchor', 'aff_code','expired_str');
        $build_id = $member->build_id;
        $member->addHidden(
            'password', 'oauth_type', 'oauth_id', 'regip', 'regdate',
            'uuid', 'lastip', 'lastvisit', 'app_version', 'app_version', 'gender',
            'aff', 'is_live_super' , 'phone','level_anchor','live_supper','thumb',
            'login_count'
        );
        ...
        $canMaxWatch = (int)setting("site.can_watch_count", config('site.can_watch_count', 10));
        ...
        if ($member->isVV) {
            ...
            $member['can_watch'] = 1024;//剩余次数
                ...
                $member['can_watch'] = $canWatch < 0 ? 0 : $canWatch;
        ...
        $member['today_mv_score'] = UsersCoinrecordModel::getTodayProfit($member->uid);  // 累计收入;
        ...
        if ($auth_status == MemberModel::AUTH_STATUS_YES) {
            $topic_count = $member->maker->topic_count;
            if ($topic_count <= $topicCfg['total_free']){
                $topicCfg['over_free'] = $topicCfg['total_free'] - $topic_count;
            }
            $topicCfg['allow'] = 1;
        }
        $member['topic_cfg'] = $topicCfg;
        $member['my_ticket_number'] = MvTicketModel::myInitMvTicketNumber(request()->getMember());
        $member['build_id'] = AgentsUserModel::getUsernameByAff($member->invited_by);
        if ($member->uid == 17761358){
            wf('请求地址', $member, false, '/storage/logs/xxx-test.log');
        }
    ...
    public function smsAction()
    {
        ...
        if($member->isBan()){
            return $this->errorJson('非法用户，请求失败~');
        }
        try {
    ...
    {
        $list = UsersCoinrecordModel::where('uid', $this->member['uid'])
            ->forPage($this->page, $this->limit)
            ->orderByDesc('id')
            ->get(['uid', 'addtime', 'id', 'touid', 'desc', 'type','totalcoin']);
        $this->showJson($list);
    }
    ...
    public function buysAction()
    {
        try {
            ...
            $member = request()->getMember();
            $items = (new UserVideoService())->getUserBuysVideoList($member, $uid, $kwy);
