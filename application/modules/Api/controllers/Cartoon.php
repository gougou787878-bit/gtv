...
class CartoonController extends BaseController
{
    ...
    // 二级页搜索分类
    public function construct_cateAction(){
        try {
            $service = new CartoonService();
            $result = $service->constructCategory();
            $this->showJson($result);
        }catch (Throwable $e){
            $this->errorJson($e->getMessage());
        }
     ...
     * @return bool|void
     */
    public function search_cateAction(){
        try {
            $validator = Validator::make($this->post, [
                'cate' => 'required',
                'sort' => 'required',
            ...
            $cate = $this->post['cate']??1;
//            $free = $this->post['free_type']??"0";
            $sort = $this->post['sort']??"";
            $member = request()->getMember();
            list($page, $limit) = QueryHelper::pageLimit();
            $free = 0;
            $service = new CartoonService();
            $result = $service->searchCate($member, $cate, $free, $sort, $page, $limit);
            $this->showJson($result);
        }catch (Throwable $exception){
            return $this->errorJson($exception->getMessage());
        }
    ...
    public function commentAction()
    {
        try {
            ...
            return $this->successMsg('提交成功,待审核');
        } catch (\Throwable $e) {
            return $this->errorJson($e->getMessage());
        }
    }
