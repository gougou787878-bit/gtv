...
class CreatorLevelSureConsole extends AbstractConsole
{
    public $name = "creator-level-sure";

    public $description = '每日计划任务-创作者等级统计';


    /**
     * php yaf daily-creator-level
     * @param $argc
     * @param $argv
     * @description
     *
    ...
    public function process($argc, $argv)
    {
        ...
        echo "start {$date} \r\n";
        echo '——————————————————————————————————————————————' . PHP_EOL;
        $maxRow = \MemberMakerModel::where('uuid','840884d3ff0d40b60c213921ea46381e')->first();
        if (is_null($maxRow)) {
            exit('查无记录');
        }
        ...
        for ($i = $maxId; $i <= $maxId; $i++) {
            ...
            if (is_null($_members)
                || $_members->vip_level == \MemberModel::VIP_LEVEL_NO
                || $_members->isBan()
            ...
            for ($k = $ct; $k >= 0; $k--) {
                list($level, $name, $vip, $vip_level,$tui_total, $mv_coins,$mv_number, $rate_percent, $rate_value) = array_values($upLevelExp[$k]);
                ...
                $_checkTui = $_members->total_tui_coins>=$tui_total;
                echo "vip:{$_checkVip} total_coin:{$_checkCoin} tui:{$_checkTui}".PHP_EOL;
                if ($_checkVip && $_checkCoin && $_checkTui) {
                    $isOk = true;
                    //同时满足 并且需要更新
                    if ($level != $_creator->level_num) {
                        ...
                        echo "创作者 ID:{$i}  UUID:{$_creator->uuid} level_num:{$level} pay_rate:{$rate_value} isOK:{$_f} maxId:{$maxId} \r\n";
                    } else {
                        echo "创作者 ID:{$i}  UUID:{$_creator->uuid} level_num:{$level} pay_rate:{$rate_value} 一致未变动 maxId:{$maxId} \r\n";
                    }
                    break;
                }
            }
