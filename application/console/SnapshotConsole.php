        ...
        \MemberModel::where('vip_level', '>=',\MemberModel::VIP_LEVEL_JIKA)
            ->where($where)->chunkById(1000, function ($items) {
                collect($items)->each(function (\MemberModel $user) {
                    ...
                    $hasSnapshot = \MemberSnapshotModel::where('uid', $user->uid)->where('created_at','>','2025-11-05')->exists();
                    if ($hasSnapshot) {
                        return;
                    }
                    ...
                    if (!is_null($order) && $order->created_at > strtotime('2025-08-05')) {
                        //3个月内 不处理
                        return;
                    }
                    ...
                    $flag = \MemberModel::where('uid', $user->uid)->update(['vip_level' => 0, 'expired_at' => 0]);
